...
namespace ShipwayBusiness.DataAccessLayer
{
    public class ServiceChargeProvider : IServiceChargeProvider
    {
        ...
        public List<ServiceChargeView> QueryAllServiceChargeView()
        {
            using(var uow= new ShipwayUnitOfWork())
            {
                ...
                if (allService.Count > 0)
                {
                    ...
                    {
                        RouterId = p.Router.RouterId,
                        KindServiceId = p.KindService.KindServiceId,
                        KindTimeReceivedId = p.KindTimeReceived.KindTimeReceivedId,
        ...
        public ServiceCharge GetServiceChargeByKindService(int router, int kindService)
        {
            using( var uow= new ShipwayUnitOfWork())
            {
                var serviceCharge = uow.ServiceChargeRepository.Find(p => p.RouterId == router && p.KindServiceId == kindService);
                if(serviceCharge == null)
                {
                    this.InsertServiceCharge(new ServiceCharge()
                    {
                        RouterId = router,
                        KindServiceId = kindService,
                        KindTimeReceivedId = 1,
                        Weigh = 1,
                        CostOderSubUrban = 1,
                        CostOderUrban = 1,
                        AddWeight = 2,
